...
public class bhdlParser {
     ...
     */
    public static String remove(String c, String source) {
        // Kivesszak a Carrige Return as Line Feed elemeket
    ...
    /**
     * find_next_Composite:visszaadja a kavetkeza Composite-ot a fajlbal
     *
     * @param source
    ...
    //Composite-ot kap és kiszedi a következõ parancsot by Peti
    public static String find_next_Command(String source) {
    ...
    //Headert szedi ki pl. main(in,out) by Peti
    public static String find_header(String source) {

        Pattern regexp = Pattern.compile(".+[^a-zA-Z]in.*(,.+?)*,out.*(,.+?)*[^a-zA-Z0-9=; |&!,_]");//Regexp tetszõleges karakterek nemABCszám karakter ( ( )in felsorolás vesszõ tetszõleges karakterekkel, out felsorolás vesszõ tetszõleges karakterekkel optimálisan amjd egy nem betûszám= ; karakter ( ) )
        // Megkeressak a talalatokat
        Matcher match = regexp.matcher(source);
        // Elsa kell
        String foundComposit = "";
